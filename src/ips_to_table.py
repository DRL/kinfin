...
def parse_interproscan(inputObj):
    ...
    for line in read_file(inputObj.interproscan_f):
        ...
        putative_gos = temp[-1]
        if not protein_id in domains_ids_by_domain_source_by_protein_id:
            domains_ids_by_domain_source_by_protein_id[protein_id] = {}
        if domain_source in domain_sources:
            if not domain_source in domains_ids_by_domain_source_by_protein_id[protein_id]:
            ...
            if putative_gos.startswith("GO:"):
                ...
                for go_term in go_terms:
                    if not 'GO' in domains_ids_by_domain_source_by_protein_id[protein_id]:
            ...
            for ipr in putative_ipr:
                if not "IPR" in domains_ids_by_domain_source_by_protein_id[protein_id]:
        ...
        for domain_source in domain_sources:
            if domain_source in domains_ids_by_domain_source_by_protein_id[protein_id]:
                if domain_source == "GO":
                    ...
                    domain_counts = Counter(domains_ids_by_domain_source_by_protein_id[protein_id][domain_source])
                ...
                if not domain_source in unique_domain_ids_by_domain_source:
