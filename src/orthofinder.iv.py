    ...
    while len(args) > 0:
        ...
        if arg == "-f" or arg == "--fasta":
            ...
            if len(args) == 0:
                print("Missing option for command line argument %s" % arg)
                util.Fail()
            options.mclInflation = []
            arg = args.pop(0)
            ivs = arg.split(",")
            print ivs
            for iv in ivs:
                try:
                    options.mclInflation.append(float(iv))
                except:
                    print("Incorrect argument for MCL inflation parameter: %s" % iv)
                    util.Fail()
...
def DoOrthogroups(options, dirs, seqsInfo):
    ...
    # 5b. MCL
    for iv in options.mclInflation:
        print("MCL clustering at I%s" % (iv))
        clustersFilename, iResultsVersion = util.GetUnusedFilename(dirs.workingDir  + "clusters_%s_I%0.1f" % (fileIdentifierString, iv), ".txt")
        MCL.RunMCL(graphFilename, clustersFilename, options.nProcessAlg, iv)
        ...
        resultsBaseFilename = dirs.resultsDir + "Orthogroups" + "_I%0.1f" % iv + ("" if iResultsVersion == 0 else "_%d" % iResultsVersion)
