...
def parse_fasta(fasta_f):
    ...
    for protein_id, protein_length in read_fasta_length(fasta_f):
        if protein_id.startswith("HSAPI"):
            protein_id = ".".join(protein_id.split(".")[0:2])
        if protein_id.startswith("MBELA"):
            protein_id = protein_id.replace(".MBELAv2", "")
...
def parse_gff(gff_f, gff_type):
    ...
    for record_type, ninth_by_key in read_gff(gff_f, gff_type):
        if gff_type == "NCBI" or gff_type == "Gnomon" or gff_type == "HGC" or gff_type == "ensembl_havana" or gff_type == "Ensembl_Metazoa" or gff_type == "BeetleBase" or gff_type == "NasoniaBase" or gff_type == "AphidBase" or gff_type == "ORCAE" or gff_type == "FlyBase":
            #print record_type, ninth_by_key
            if record_type == 'CDS':
                mRNA_id = ninth_by_key['Parent'].replace("transcript:", "")
                protein_id = ninth_by_key['protein_id']
                if protein_id in proteinCollection.protein_length_by_protein_id:
                #print gene_id_by_mRNA_id
                    if mRNA_id in gene_id_by_mRNA_id:
                        gene_id = gene_id_by_mRNA_id[mRNA_id]
                        annotationCollection.add_mRNA(gene_id, protein_id)
            elif record_type == 'mRNA':
                gene_id = ninth_by_key['Parent'].replace("gene:", "")
            ...
            if record_type == 'gene':
                gene_id = ''
                if gff_type == "OIST_MGU":
                    gene_id = ninth_by_key['ID'].replace("gene:", "")
                else:
                ...
                elif gff_type == "TRIA" or gff_type == "OIST_MGU":
                    mRNA_id = ninth_by_key['ID'].replace("transcript:", "")
                elif gff_type == "OIST":
                    mRNA_id = ninth_by_key['ID'].replace("transcript:", "") + ".p"
                elif gff_type == "AUGUSTUS":
